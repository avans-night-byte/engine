...
Input SDLInputEngineAdapter::getInput() {
    ...
    while (SDL_PollEvent(&e)) {
        if (e.type == SDL_KEYDOWN) return getKeyInput(e.key.keysym.sym);
        if (e.type == SDL_MOUSEBUTTONDOWN) return getMouseInput(e);
        if (e.type == SDL_JOYBUTTONDOWN) return getControllerInput(e);
    }
...
Input SDLInputEngineAdapter::getControllerInput(SDL_Event controllerEvent) {
    ...
    return Input{.device = Input::CONTROLLER, .x = -1, .y = -1};
}
